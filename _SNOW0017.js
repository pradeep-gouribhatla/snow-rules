...
module.exports = function(context) {
    ...
    var _getArgumentsList = function(node) {
        return node.parent ? (node.parent.arguments ? node.parent.arguments : null) : null;
    };
    ...
    var _checkForSetValue = function(nodePropertyName, node) {
        if (nodePropertyName && nodePropertyName === SETVALUE) {
            var argumentsList = node.parent ? (node.parent.arguments ? node.parent.arguments : null) : null;
            if (argumentsList && argumentsList.length !== 2) {
                ...
                (!_checkStringValidity(argumentsList[0]) || !_checkStringValidity(argumentsList[1]))
            ) {
    ...
    return {
        condition(record) {
            return record.sys_class_name == "sys_script_include" || record.sys_class_name == "sys_script";
        },
